...
def main(perfume_idx_list):
    ...
    for perfume_idx in perfume_idx_list:
        ...
        perfume = PerfumeEntity(perfume_idx=perfume_idx, name=None, english_name=None, story=None,
                                volume_and_price=None,
                                abundance_rate=None, image_url=image_url)
        print(perfume.__dict__)
        affected_rows = perfume_model.update(perfume.__dict__)
        if affected_rows == 0:
